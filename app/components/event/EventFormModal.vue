...
import UiTextarea from '~/components/ui/UiTextarea.vue' // ⭐️ เพิ่ม Textarea
...
// ⭐️ ฟอร์มนี้ตรงกับตาราง `events` ใน SQL
const form = reactive({
  id: null,
  name: '',
  description: '',
...
// ⭐️ Watcher นี้จะ map props.event ที่มีฟิลด์ตรงกันเป๊ะๆ
watch(() => props.event, (e) => {
  if (e) {
    form.id = e.id ?? null
    form.name = e.name ?? ''
...
<template>
  <div v-if="show" class="fixed inset-0 flex items-start justify-center p-4 pt-20 z-[100] bg-black/50 backdrop-blur-sm overflow-y-auto">
    ...
    <div class="relative w-full max-w-2xl bg-white text-text-primary rounded-xl shadow-2xl flex flex-col max-h-[85vh]">
      ...
      <div class="p-6 space-y-4 overflow-y-auto">
        <UiInput v-model="form.name" label="ชื่ออีเวนต์" />
        <UiTextarea v-model="form.description" label="รายละเอียด" />
        ...
        <UiInput v-model="form.client_name" label="ลูกค้า" />
        <UiInput v-model="form.location" label="สถานที่ (ประเทศ)" />
        <UiInput v-model.number="form.total_budget" label="งบประมาณทั้งหมด (THB)" type="number" />
        
        <hr class="border-gray-200" />
        
        <h4 class="text-md font-semibold text-gray-800">รายละเอียดสถานที่</h4>
        <UiInput v-model="form.venue_name" label="ชื่อสถานที่จัดงาน" />
        <UiInput v-model="form.venue_url" label="ลิงก์สถานที่ (Venue URL)" placeholder="https://..." />
        
        <h4 class="text-md font-semibold text-gray-800">รายละเอียดที่พัก</h4>
        <UiInput v-model="form.accommodation_name" label="ชื่อที่พัก" />
        <UiInput v-model="form.accommodation_url" label="ลิงก์ที่พัก (Accommodation URL)" placeholder="https://..." />
        
        <hr class="border-gray-200" />
        
        <UiInput v-model="form.drive_link" label="ลิงก์ไดรฟ์ (Google Drive)" placeholder="https://drive.google.com/..." />
      </div>
